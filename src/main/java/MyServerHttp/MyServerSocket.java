...
public class MyServerSocket extends Thread {
	...
	static long i = 0;
	public static Logger logger = Logger.getLogger(MyServerSocket.class);
	...
	public MyServerSocket(Socket accept) {
		this.accept = accept;
	}
	...
	public static void main(String[] args) {
		...
		try {
			...
			logger.info("外网ip：" + net_address);
			logger.info("外网端口：" + net_port);
          
			serverSocket = new ServerSocket(Integer.parseInt(server_port));
			logger.info("代理服务器开启成功!");
			
			while (true) {
				Socket accept = serverSocket.accept();
				MyServerSocket aa = new MyServerSocket(accept);
				aa.start();
			}

		} catch (IOException e) {
			logger.error("代理服务器开启失败!");
	...
	public void connect() {
		try {
			...
			socket = new Socket(net_address, Integer.parseInt(net_port));
